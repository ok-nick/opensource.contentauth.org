...
The C2PA **[Verify tool](https://contentcredentials.org/verify)** uses a list of _known certificates_ to determine that a Content Credential was issued by a known source. If an asset's Content Credential was not signed by a certificate on the list (as explained below), the Verify tool will display this message:
...
Conversely, if the Content Credential was signed by a certificate on the list, the Verify tool will display the [name of the certificate owner and time of the claim signature](verify#title-and-signing-information).
...
Please email `tbd@contentcredentials.org` with the pertinent information.
