...
:::note
This information was updated in **June 2024**. It will be regularly updated but may not always reflect the most current status.
:::

Tasks in progress or planning are categorized as follows (in reverse order of progression):
...
A new issue/task starts in the **In Review / Triage** status. Once reviewed and approved, it moves into **To Do** status, and when a developer begins working on it, into **In Progress**. Once development work is done, the task moves to **Code Complete**. After testing and validation, it moves to **Merged to Main** and finally when the code is put into a release&mdash; to **Released**.
