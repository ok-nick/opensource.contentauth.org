...
You can also display Content Credentials information for an asset with a publicly-visible URL by using a URL with the following format:
...
If the asset was created with Adobe tools and the asset's creator has [connected social media accounts](https://connected-accounts.adobe.com/), then the **Credit and usage** section shows the information for Behance, Instagram, LinkedIn,and X (Twitter).
...
The `identifier` property is the DID of the social media account, and the `name` property is the handle. The `credential` property is a [Credential](https://www.w3.org/TR/vc-data-model/#credentials) that contains a hash of the DID and the URL of the Credential itself.

<div class="review-comment">
  Is the above statement right? Can we provide any more useful info on ho to
  write this part of the manifest?
</div>

The CreativeWork assertion from manifest JSON for the above example is shown below.
