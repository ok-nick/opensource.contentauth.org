...
The top of the right panel displays a thumbnail of the image with the title of the asset from the `title` property in the active manifest. The value of this property might not be the same as the file name. For example, suppose you downloaded a file from Adobe Stock and renamed it `my_stock_download.jpeg`. In Verify it could appear as follows:
